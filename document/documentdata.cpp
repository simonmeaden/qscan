...
const QString DocumentDataStore::LANGUAGE = "language";
const QString DocumentDataStore::PAGES = "pages";
...
QString DocumentDataStore::lang() const
{
  return m_lang;
}

void DocumentDataStore::setLang(const QString& lang)
{
  m_lang = lang;
}
...
void DocumentDataStore::load(const QString& filename)
{
  ...
  if (file.exists()) {
    ...
    if (doc_node[LANGUAGE]) {
      m_lang = doc_node[LANGUAGE].as<QString>();
    }

    if (doc_node[PAGES]) {
      YAML::Node pages_node = doc_node[PAGES];

      if (!pages_node.IsNull()) {
        if (pages_node.IsMap()) {
          for (YAML::const_iterator it = pages_node.begin();
               it != pages_node.end();
               ++it) {
            ...
            if (item[TEXT_LIST]) {
              ...
              if (text_node.IsMap()) {
                for (YAML::const_iterator string_it = text_node.begin();
                     string_it != text_node.end();
                     ++string_it) {
                      ...
                      if (text_node[TEXT_STYLES]) {
                        ...
                        if (styles_node.IsSequence()) {
                          for (std::size_t i = 0; i < styles_node.size(); i++) {
                            ...
                            style->setType(StyleData::Type(
                                             style_node[TEXT_STYLE].as<int>()));
            ...
            if (item[IMAGE_LIST]) {
              ...
              if (images_node.IsMap()) {
                for (YAML::const_iterator it = images_node.begin();
                     it != images_node.end();
                     ++it) {
                  int key = it->first.as<int>();
                  QString text(it->second.as<std::string>().c_str());
                  images.insert(key, DocUtil::cleanText(text));
                }
              }
            }
            ...
            if (item[IMAGE_NAMES]) {
              ...
              if (image_name_node.IsMap()) {
                for (YAML::const_iterator it = image_name_node.begin();
                     it != image_name_node.end();
                     ++it) {
                  int key = it->first.as<int>();
                  QString text(it->second.as<std::string>().c_str());
                  image_names.insert(key, DocUtil::cleanText(text));
                }
              }
            }
        ...
        if (doc_data) {
          ...
          if (doc_data->isRemoveTextLater()) {
            ...
            emitter << YAML::BeginMap; // begin of text map
            {
              ...
              for (auto string_it = string_map.begin();
                   string_it != string_map.end();
                   ++string_it) {
                ...
                emitter << YAML::BeginMap;
                {
                  ...
                  emitter << YAML::Value
                          << DocUtil::cleanText(string_it.value());
                  ...
                  emitter << YAML::BeginSeq;
                  {
                    ...
                    for (auto style_it = styles.begin();
                         style_it != styles.end();
                         style_it++) {
                      emitter << YAML::BeginMap;
          ...
          for (auto image_it = image_map.keyBegin(), end = image_map.keyEnd();
               image_it != end;
               ++image_it) {
          ...
          for (auto image_it = image_names.keyBegin(),
               end = image_names.keyEnd();
               image_it != end;
               ++image_it) {
            emitter << YAML::Key << *image_it;
...
//DocData::DocData(DocData const& other)
//{
//  m_page_no = other.m_page_no;
//  m_filename = other.m_filename;
//  m_remove_image_later = other.m_remove_image_later;
//  m_remove_text_later = other.m_remove_text_later;
//  m_inverted = other.m_inverted;
//  m_completed = other.m_completed;
//  m_text_has_changed = other.m_text_has_changed;
//  m_images_changed = other.m_images_changed;
//  m_text_initialised = other.m_text_initialised;
//}
...
void DocData::setText(int index, const StyledString& text)
{
  ...
  QString cleaned = DocUtil::cleanText(text.text());
  m_text_list.clear();
