...
void
OcrEditor::setResolution(const QImage& image)
{
  int x_dpi = DocUtil::dpmToDpi(image.dotsPerMeterX());
  int y_dpi = DocUtil::dpmToDpi(image.dotsPerMeterY());
  setResLabel(x_dpi, y_dpi);
}

void
OcrEditor::setData(int page_no, const QImage& image, const DocumentData& doc_data, const QString& lang)
{
  m_page_no = page_no;
  m_doc_data = doc_data;
  m_lang = lang;
...
void
OcrEditor::setOcrImage(int page_no, const QImage& image)
{
...
void
OcrEditor::setOcrText(int page_no, const QString& text)
{
...
void
OcrEditor::appendOcrText(int page_no, const QString& text)
{
  if (page_no == m_page_no) {
...
void
OcrEditor::ocrFailed(int /*page_no*/)
{
  QMessageBox::warning(this,
                       tr("OCR Failure"),
                       tr("Unfortunately your ORC request produced no text.\n"
                          "Try tweaking the image and try again."),
                       QMessageBox::Ok);
}
...
QImage
OcrEditor::modifiedImage()
{
  return m_image_display->currentImage();
}

int
OcrEditor::pageNumber()
{
  return m_page_no;
}

DocumentData
OcrEditor::documentData() const
{
  return m_doc_data;
}
...
QFrame*
OcrEditor::initDoubleSliderFrame()
{
...
QFrame*
OcrEditor::initDenoiseSliderFrame()
{
...
void
OcrEditor::requestOcrOnSelection()
{
...
void
OcrEditor::scanListWasClicked(const QModelIndex& /*index*/)
{
...
void
OcrEditor::scanListWasDoubleClicked(const QModelIndex& index)
{
...
void
OcrEditor::removeItemFromDocument()
{
...
void
OcrEditor::ocrImageChanged(bool changed)
{
...
OcrEditor::applyValue()
{
  ...
  case Rescale: {
    qreal value = DocUtil::round(m_dblvalue_slider->value(), 2);
  ...
  case Denoise: {
    ...
    int template_value = DocUtil::makeOdd(m_denoise_template_slider->value());
    int search_value = DocUtil::makeOdd(m_denoise_search_slider->value());
...
void
OcrEditor::setCurrentChangeState()
{
...
void
OcrEditor::setIntValueLabel(qreal value)
{
  m_intvalue_lbl->setText(QString::number(int(value)));
}

void
OcrEditor::setDoubleValueLabel(qreal value)
{
  m_dblvalue_lbl->setText(tr("%1").arg(value, 0, 'g', 2));
}

void
OcrEditor::setDenoiseTemplateLabel(qreal value)
{
  m_denoise_template_lbl->setText(QString::number(DocUtil::makeOdd(value)));
}

void
OcrEditor::setDenoiseSearchLabel(qreal value)
{
  m_denoise_search_lbl->setText(QString::number(DocUtil::makeOdd(value)));
}

void
OcrEditor::setDenoiseFilterLabel(qreal value)
{
  m_denoise_filter_lbl->setText(QString::number(int(value)));
}

void
OcrEditor::valueAccepted()
{
  m_ctl_stack->setCurrentIndex(m_btn_stack);
}

void
OcrEditor::disableBinarise()
{
  m_greyscale_btn->setEnabled(false);
  enableCleanImageBtns(true);
}
...
void
OcrEditor::enableBinarise()
{
  m_greyscale_btn->setEnabled(true);
}

void
OcrEditor::enableCleanImageBtns(bool enable)
{
...
void
OcrEditor::imageSizeHasChanged(int width, int height, int xres, int yres)
{
...
void
OcrEditor::resizeEvent(QResizeEvent* event)
{
...
void
OcrEditor::setMessageLabel(const QString& text)
{
  m_message_lbl->setText(text);
}

void
OcrEditor::setSizeLabel(int width, int height)
{
  m_image_size_lbl->setText(m_image_size.arg(width).arg(height));
}

void
OcrEditor::setNameLabel(int page, QString name)
{
  m_image_name_lbl->setText(m_image_name.arg(page).arg(name));
}

void
OcrEditor::setResLabel(int xres, int yres)
{
