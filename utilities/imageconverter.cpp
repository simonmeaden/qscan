...
PIX* imageToPix(const QImage& image)
{
  ...
  pixSetXRes(pixs, DocUtil::dpmToDpi(image.dotsPerMeterX()));
  pixSetYRes(pixs, DocUtil::dpmToDpi(image.dotsPerMeterY()));
...
QImage pixToImage(PIX* pixImage)
{
  ...
  result.setDotsPerMeterX(DocUtil::dpiToDpm(pixImage->xres));
  result.setDotsPerMeterY(DocUtil::dpiToDpm(pixImage->yres));
