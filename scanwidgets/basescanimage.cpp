...
void BaseScanImage::mouseMoveEvent(QMouseEvent* event)
{
  if (!m_current_image.isNull()) {
    ...
    if (m_is_inside) {
      ...
      case EDGE_DRAWING:
        qInfo() << tr("Edge moving. (%1, %2)").arg(m_edge_finish.x()).arg(m_edge_finish.y());
...
void BaseScanImage::mouseReleaseEvent(QMouseEvent* event)
{
  if (!m_current_image.isNull()) {
    ...
    case RUBBERBANDING: {
      ...
      qDebug() << tr("x:%1, y:%2, width:%3, height:%4")
               .arg(m_rubber_band.x())
               .arg(m_rubber_band.y())
...
{
  qInfo() << tr("Rotating by %1°").arg(angle);
